...
PREFER_RUNTIME_HANDOFF_BASE64="${PREFER_RUNTIME_HANDOFF_BASE64:-${VLLM_RUNTIME_HANDOFF_BASE64:-}}"
...
if [ -n "${PREFER_RUNTIME_HANDOFF}${PREFER_RUNTIME_HANDOFF_BASE64}" ]; then
  if [ -n "${PREFER_BUNDLE:-}${PREFER_MODELS:-}" ]; then
    echo "[vllm-entrypoint] a runtime handoff cannot be combined with bundle or model selectors" >&2
    exit 2
  fi
  mkdir -p /run/prefer
  ...
  unset PREFER_RUNTIME_HANDOFF_BASE64 VLLM_RUNTIME_HANDOFF_BASE64
