...
def server_command(lane: dict, server: dict) -> list[str]:
    ...
    for key, flag in (
        ...
        if value is None:
            continue
        command.extend([flag, str(value)])
    lora_paths = server.get("lora_paths", [])
    if not isinstance(lora_paths, list) or any(not isinstance(value, str) or not value for value in lora_paths):
        raise ValueError(f"{lane['key']}: lora_paths must contain strings")
    if lora_paths:
        command.append("--lora-paths")
        command.extend(lora_paths)
...
def resolve_handoff_base(base: str, default_base: str, handoff: dict, primary_by_key: dict[str, dict]) -> tuple[dict, str]:
    ...
    if normalized == "default":
        return default_server_overrides(), "sglang/cuda13"
    if explicit.is_file():
        config = load_json(explicit)
        server = copy.deepcopy(config.get("server", {}))
        for name in (
            "served_model_name", "model_path", "model_revision", "model_variant",
            "component_weights_paths", "quantization", "load_format", "reasoning_parser",
            "tool_call_parser", "trust_remote_code", "runtime_mode", "lora_paths", "extra_args",
        ):
    ...
    if scenario is None:
        raise ValueError(f"unknown SGLang runtime handoff deployment: {reference!r}")
    return copy.deepcopy(scenario["server"]), PurePosixPath(normalized).with_suffix("").as_posix()


def handoff_sglang_lane(handoff: dict, model: dict, artifacts: dict[str, dict]) -> dict:
    ...
    protected = {
        "model_path", "model_revision", "component_weights_paths", "served_model_name",
        "lora_path", "lora_weight_name", "lora_paths",
    } & launcher.keys()
    if protected:
    ...
    runtime_mode = str(launcher.get("runtime_mode", model_settings.get("runtime_mode", "text")))
    launcher.pop("runtime_mode", None)
    if runtime_mode == "text":
        ggufs = [
        ...
        lora_paths = handoff_lora_modules(selected_artifacts, model_id)
        if lora_paths:
            launcher["lora_paths"] = lora_paths
    else:
        container_path = repository_path
        launcher["model_path"] = repository_path
        launcher["model_revision"] = model.get("revision")
        component_roles = {
            "fl2va-transformer": "transformer",
        ...
        for artifact in selected_artifacts:
            ...
            component = component_roles.get(role)
            settings = artifact.get("settings", {})
            if isinstance(settings, dict) and isinstance(settings.get("component"), str):
                component = settings["component"]
            if component:
                component_paths[component] = artifact["local_path"]
            if role in {"lora", "adapter"}:
                lora_path = PurePosixPath(artifact["local_path"])
                launcher["lora_path"] = str(lora_path.parent)
                launcher["lora_weight_name"] = lora_path.name
        if component_paths:
            launcher["component_weights_paths"] = component_paths

    task = str(model_settings.get("task", "chat" if runtime_mode == "text" else "video"))
    tasks = model_settings.get("tasks", [task])
    ...
    if not isinstance(modalities, dict):
        raise ValueError(f"runtime handoff model {model_id} modalities must be an object")
    return {
        "key": model_id,
        "id": model_id,
        "request_model_id": request_model_id,
        ...
        "task": task,
        "tasks": tasks,
        "mode": model_settings.get("mode", runtime_mode),
        "capability_aliases": model_settings.get("capability_aliases", {}),
...
def compose_runtime_handoff_config(
    *,
    ...
    if handoff.get("schema_version") != "prefer.runtime-handoff.v1" or handoff.get("engine") != "sglang":
        raise ValueError("materialized runtime handoff is not for SGLang")
    ...
    if not isinstance(models, list) or len(models) != 1 or not isinstance(raw_artifacts, list) or not raw_artifacts:
        raise ValueError("SGLang runtime handoff requires exactly one model and at least one artifact")
    ...
    lane = handoff_sglang_lane(handoff, models[0], artifacts)
    ...
    protected = {"model_path", "model_revision", "component_weights_paths", "served_model_name", "lora_paths"}
...
def main() -> None:
    ...
    if args.compose_handoff:
        ...
        print(f"composed SGLang runtime config from immutable handoff {plan['handoff_fingerprint']}")
        return
    if args.compose:
