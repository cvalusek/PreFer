    ...
    for key, model in models.items():
        ...
        for download in downloads:
            ...
            if not isinstance(revision, str) or not re.fullmatch(r"[0-9a-fA-F]{40}", revision):
                raise CatalogError(f"{key}: download revision must be an immutable 40-character commit SHA")
        ...
        for artifact in artifacts:
            ...
            if not isinstance(artifact.get("size"), int) or artifact["size"] <= 0:
                raise CatalogError(f"{key}: artifact needs a positive byte size: {artifact['path']}")
            if not re.fullmatch(r"[0-9a-fA-F]{64}", str(artifact.get("sha256", ""))):
                raise CatalogError(f"{key}: artifact needs a SHA-256 digest: {artifact['path']}")
...
def model_download_fingerprint(key: str, model: dict[str, Any]) -> str:
    payload = {"schema_version": 2, "key": key, "downloads": model["downloads"], "artifacts": model["artifacts"]}
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(encoded).hexdigest()


def artifact_identity(artifact: dict[str, Any], revision: str) -> str:
    payload = {
...
def render_downloads(catalog: dict[str, Any]) -> str:
    records: OrderedDict[str, tuple[dict[str, Any], str]] = OrderedDict()
    model_ids: OrderedDict[str, list[str]] = OrderedDict()
    for key, model in catalog["models"].items():
        revisions = {download["repo"]: download["revision"] for download in model["downloads"]}
        ids: list[str] = []
        for artifact in model["artifacts"]:
            revision = revisions.get(artifact["repo"])
            if not isinstance(revision, str) or not re.fullmatch(r"[0-9a-f]{40}", revision):
                raise CatalogError(f"{key}: artifact repository {artifact['repo']} requires an immutable revision")
            artifact_id = artifact_identity(artifact, revision)
            existing = records.get(artifact_id)
            if existing and existing != (artifact, revision):
                raise CatalogError(f"{key}: artifact identity collision for {artifact['repo']}/{artifact['path']}")
            records[artifact_id] = (artifact, revision)
    ...
    lines = [
        ...
        "set -euo pipefail",
        "",
        f"readonly LLAMA_GENERATED_MODEL_KEYS={shlex.quote(','.join(catalog['models']))}",
        "",
        "llama_model_artifact_ids() {",
        "  case \"$1\" in",
    ]
    for key, ids in model_ids.items():
        quoted = " ".join(shlex.quote(item) for item in ids)
        lines.extend([f"    {shlex.quote(key)})", f"      printf '%s\\n' {quoted}", "      ;;"])
    lines.extend(
        [
            "    *) echo \"[download-models] unknown model key: $1\" >&2; return 2 ;;",
            "  esac",
            "}",
            "",
            "llama_artifact_record() {",
            "  case \"$1\" in",
        ]
    )
    for artifact_id, (artifact, revision) in records.items():
        values = [
            artifact_id,
            artifact["repo"],
            revision,
    ...
    lines.extend(
        [
            "    *) echo \"[download-models] unknown artifact id: $1\" >&2; return 2 ;;",
            "  esac",
            "}",
            "",
        ]
...
def render_inventory(
    ...
    inventory = OrderedDict(
        ...
        composition=OrderedDict(
            ...
            activation="required; select models or supply an immutable runtime handoff",
            multi_model=True,
            ...
            selection_rules=[
                "models are selected explicitly at runtime",
                "an exact quant key replaces another lane for the same logical model",
                "a friendly model selection uses its unique catalog lane",
            ...
            setting_sources=OrderedDict(
                server="runtime server overrides",
                model="models[].settings and runtime model overrides",
            ),
        ),
        model_profiles=model_profiles,
        models=inventory_models,
...
def expected_outputs() -> dict[Path, str]:
    ...
    return {
        ...
        INVENTORY_PATH: render_inventory(catalog, []),
    }
...
def compose_runtime_preset(
    *,
    ...
    if base:
        raise CatalogError("hardware deployments and preset bases have been removed")
    if parse_composition_list(bundle_value):
        raise CatalogError("llama.cpp bundles have been removed; select models explicitly")

    catalog = load_catalog()
    validate_catalog(catalog)
    requested_models = parse_composition_list(model_value)
    if not requested_models:
        raise CatalogError("runtime composition requires at least one PREFER_MODELS selection")
    ...
    for selection in requested_models:
        matches = [
            key for key, model in catalog["models"].items()
        ...
        slug = catalog["models"][key]["_catalog"]["model_slug"]
        selected = [
            entry for entry in selected
            if catalog["models"][entry["key"]]["_catalog"]["model_slug"] != slug
        ]
        selected.append(OrderedDict(key=key))

    composed = OrderedDict(
        path="runtime/effective.ini",
        defaults=OrderedDict(server_overrides),
        models=selected,
    )
    applied_model_overrides: OrderedDict[str, Any] = OrderedDict()
    for entry in selected:
        key = entry["key"]
        names = llama_selection_names(key, catalog["models"][key])
        ...
        if matching:
            entry["overrides"] = OrderedDict(matching[0])
            applied_model_overrides[key] = matching[0]
    unknown = [
        name for name in model_overrides
        if not any(name in llama_selection_names(entry["key"], catalog["models"][entry["key"]]) for entry in selected)
    ]
    if unknown:
        raise CatalogError("PREFER_MODEL_OVERRIDES targets are not selected: " + ", ".join(unknown))
...
def compose_runtime_handoff_preset(
    *,
    ...
    if base or default_base or handoff.get("base_deployment"):
        raise CatalogError("runtime handoffs no longer accept a base deployment or preset")
    ...
    for model in models:
        ...
        settings = runtime_handoff_model_settings(handoff, model, artifacts, OrderedDict())
        for source in (handoff_server, server_overrides):
            for name, value in source.items():
                if value is None:
                    settings.pop(name, None)
...
def main() -> int:
    ...
    if args.compose:
        ...
        try:
            ...
            plan_output.write_text(json.dumps(plan, indent=2) + "\n", encoding="utf-8", newline="\n")
            print("composed llama.cpp runtime preset: " + ", ".join(prestage))
            return 0
        except (CatalogError, KeyError, TypeError, OSError, json.JSONDecodeError) as exc:
            print(f"generate-presets: {exc}", file=sys.stderr)
            return 2
