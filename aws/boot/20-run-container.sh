...
ENV_ARGS=()
for v in S3_BUCKET_NAME S3_MODEL_PREFIX SGLANG_S3_BUCKET_NAME SGLANG_S3_MODEL_PREFIX HF_TOKEN PRESTAGE_MODELS MODEL_CACHE_RECHECK_DAYS MODEL_DOWNLOAD_JOBS LLAMA_ARG_MODELS_PRESET LLAMA_ARG_MODELS_MAX AWS_REGION; do
  if [ -n "${!v:-}" ]; then
    ENV_ARGS+=(-e "$v=${!v}")
  fi
done
